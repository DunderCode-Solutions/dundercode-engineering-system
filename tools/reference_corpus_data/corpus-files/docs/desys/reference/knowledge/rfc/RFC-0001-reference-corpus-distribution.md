...
The first v0.2 alpha bundle supports initial installation and reconciliation of
the same packaged snapshot. A manifest whose bundle checksum differs from the
installed package fails closed before its ownership entries are trusted.
Cross-snapshot update, addition, and removal remain an implementation gate until
packages carry trusted predecessor descriptors. The table below defines the
required behavior when that gate is implemented; the alpha does not silently
approximate it.
...
The first alpha implementation is complete when:
...
- manifests from any other bundle checksum fail closed before ownership is trusted;
...
Cross-snapshot update support is a separate, unmet implementation gate. It is
complete only when trusted predecessor descriptors validate prior ownership and
automated tests prove that safe upgrades preserve provenance, update unchanged
files, add new files, remove obsolete unchanged files, and refuse every locally
modified or deleted file before any write.
