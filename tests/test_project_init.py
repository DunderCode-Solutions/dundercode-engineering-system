...
def test_initializes_a_loadable_consumer_configuration(tmp_path: Path) -> None:
    ...
    assert (root / "tools/desys-source.txt").read_text(encoding="utf-8") == (
        "dundercode-engineering-system==0.2.0a1\n"
    )
...
def test_installs_reference_corpus_manifest_legal_files_and_sources(tmp_path: Path) -> None:
    ...
    wheel = vendor / "dundercode_engineering_system-0.2.0a1-py3-none-any.whl"
    ...
    assert manifest["release_tag"] == bundle.release_tag == "v0.2.0-alpha.1"
    assert manifest["source_commit"] == bundle.source_commit == "1ba18c126dc9adf035f64c0ca6eda75186e73b60"
...
def test_quality_script_accepts_source_without_terminal_newline(tmp_path: Path) -> None:
    ...
    initialize_project(root, version=TEST_VERSION)
    source_file = root / "tools/desys-source.txt"
    source_file.write_text("dundercode-engineering-system==0.2.0a1", encoding="utf-8")
    ...
    assert marker.read_text(encoding="utf-8").count("dundercode-engineering-system==0.2.0a1") == 3
